    ...
    public partial class Purchase_History : Form
    {
        ...
        public void databind(string dtstartvl,string dtendvl)
        {
            try
            {
                ...
                {
                   sql = " select id as ID, purchase_date as 'Date',  product_name as 'Name', cost_price as 'Price', product_quantity as 'Qty', " +
                ...
                {
                    sql = " select id as 'الرقم التعريفي', purchase_date as 'تاريخ الشراء',  product_name as 'اسم المنتج', cost_price as 'تكلفة النتج', product_quantity as 'الكمية', " +
                         "  ((cost_price * product_quantity) * 1.00) as 'الاجمالي',  category as 'الصنف' , supplier as 'المورد', Shopid as 'رقم المحل' , ptype as 'الحالة'  " +
                         "  from tbl_purchase_history  where purchase_date   BETWEEN '" + dtstartvl + "' AND    '" + dtendvl + "' " +
                         " and status = 1 Order  by id desc";
                }
