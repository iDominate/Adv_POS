...
namespace supershop
{
    public partial class salesreport : Form
    {
        ...
        private void dtDailyPaymentReport_ValueChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        sql = "select  sales_id as 'Rpt No' , sales_time as 'Date' , payment_amount as 'Total' , emp_id as 'Sold by',  dis as 'Dis' , " +
                    ...
                    {
                        sql = "select  sales_id as 'رقم التقرير' , sales_time as 'التاريخ' , payment_amount as 'الاجمالي' , emp_id as 'بيع من قبل',  dis as 'Dis' , " +
                                    " vat as 'الضريبة' ,  payment_type as 'طريقة الدفع' ,  due_amount as 'المبلغ المستحق', c_id as 'رقم العميل' , Comment as 'ملاحظات' " +
                                    " from sales_payment where sales_time   like  '%" + dtDailyPaymentReport.Text + "%' order by sales_time";
            ...
            else
            {
                try
                {
                    string sql;
                     if(Thread.CurrentThread.CurrentCulture.Parent.Name.Equals("en"))
                    {
                         sql = "select  sales_id as 'Receipt No' , sales_time as Date, item_id as 'Item ID', itemName as 'Item Name' , " +
                    ...
                    {
                        sql = "select  sales_id AS 'رقم الفاتورة' , sales_time AS 'التاريخ', item_id AS 'رقم المنتج', itemName AS 'اسم المنتج' , " +
                                 " RetailsPrice AS 'تكلفة المنتج' ,Qty AS 'الكمية' , Total AS '-الاجمالي-',  profit * Qty AS 'الربح'  " +
                                 " from sales_item where  itemName   like  '" + txtItemSearchBox.Text + "%' " +
                                 " or sales_id   =  '" + txtItemSearchBox.Text + "' " +
                                 " and status = 1 or status = 3  order by sales_time";
            ...
            else
            {
                try
                {
                    ...
                    string sql;
                     if(Thread.CurrentThread.CurrentCulture.Parent.Name.Equals("en"))
                    {
                        sql = " select   sales_time as 'Date' , itemName as 'Name', RetailsPrice  as 'Price', Qty,  Total,  " +
                    ...
                    {
                        sql = " select   sales_time as 'التاريخ' , itemName as 'اسم المنتج', RetailsPrice  as 'السعر', Qty,  Total,  " +
                                    " ((profit * Qty) * 1.00) as 'الربح'   , discount as 'Dis Rate', sales_id as 'رقم التقرير' " +
                                    " from sales_item where sales_time BETWEEN '" + dtSalesItemStart.Text + "' AND    '" + dtSalesItemEND.Text + "' " +
                                    " and status != 2  Order  by sales_time";
                    }
                    DataAccess.ExecuteSQL(sql);
                    ...
                    if(Thread.CurrentThread.CurrentCulture.Parent.Name.Equals("en"))
                    {
                         sql3 = " select SUM(Total) , SUM(profit * Qty) as Profit , SUM(discount)   " +
                                    " from sales_item where sales_time BETWEEN '" + dtSalesItemStart.Text + "' AND    '" + dtSalesItemEND.Text + "' " +
                                    " and status != 2  Order  by sales_time";
                    }else
                    {
                        sql3 = " select SUM(Total) as 'الاجمالي' , SUM(profit * Qty) as 'الربح' , SUM(discount)   " +
                                    " from sales_item where sales_time BETWEEN '" + dtSalesItemStart.Text + "' AND    '" + dtSalesItemEND.Text + "' " +
                                    " and status != 2  Order  by sales_time";
                    }
        ...
        private void dtEndDate_ValueChanged(object sender, EventArgs e)  ///////Search From
        {
            ...
            else
            {
                try
                {
                    ...
                    if (Thread.CurrentThread.CurrentCulture.Parent.Name.Equals("en"))
                    {
                        sql = "select   sales_time as 'Date' , SUM(payment_amount) as 'Total' , SUM(dis) as 'Discount' , SUM(vat) as 'TAX'  ,  " +
                    ...
                    {
                        sql = "select   sales_time as 'التاريخ' , SUM(payment_amount) as 'الاجمالي' , SUM(dis) as 'الخصم' , SUM(vat) as 'الضريبة'  ,  " +
                                 "SUM(due_amount)  as 'المبلغ المستحق'  from sales_payment where sales_time BETWEEN '" + dtStartDate.Text + "' AND    '" + dtEndDate.Text + "' " +
                                 " GROUP BY  sales_time  Order  by sales_time";
        ...
        private void dateTimePicker6Return_ValueChanged(object sender, EventArgs e)  
        {
            ...
            else
            {
                try
                {
                    ...
                    if(Thread.CurrentThread.CurrentCulture.Parent.Name.Equals("en"))
                    {
                        sql = "select  return_time as 'Date' , itemName as 'itemName', RetailsPrice  as 'Price', Qty,  Total , custno as CustID, " +
                    ...
                    {
                        sql = "select  return_time as 'التاريخ' , itemName as 'اسم المنتج', RetailsPrice  as 'السعر', Qty,  Total , custno as 'رقم العميل', " +
                                " SoldInvoiceNo as 'رقم الفاتورة' , emp as 'ارجع من قبل'  from return_item where return_time " +
                                " BETWEEN '" + dtReturnStartDate.Text + "' AND    '" + dtReturnEndDate.Text + "' Order  by return_time";
                    }
        ...
        private void dtgrdViewSalesReport_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
