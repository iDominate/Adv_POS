...
namespace supershop 
{
    public partial class SalesRegister : Form
    {
        ...
        private void SalesRegister_Load(object sender, EventArgs e)
        {
            ...
            foreach (TabPage item in tabSRcontrol.TabPages)
            {
              ...
              //  " ORDER BY NEWID() "; // SQL server and use -- top 12 after select  
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                  ...
                  //  toolTip1.SetToolTip(b, details);          //Umncomment
                    ...
                    itemStock.Add(dataReader["product_id"].ToString(), int.Parse(dataReader["product_quantity"].ToString()));
                  ...
                  //  b.Text += "\n Buy: " + dataReader["cost_price"];
                    ...
                    {
                        b.Text += "\n Stock: " + itemStock[dataReader["product_id"].ToString()];
                        b.Text += "\n R.Price: " + dataReader["retail_price"];
                        b.Text += "\n Dis: " + dataReader["discount"] + "% ";
                    }
                    else
                    { 
                        
                        //dataReader["product_quantity"]
                        b.Text += "\n المخزون: " + itemStock[dataReader["product_id"].ToString()];
                        b.Text += "\n سعر : " + dataReader["retail_price"];
                        b.Text += "\n خصم: " + dataReader["discount"] + "% ";
                    }
                    
                    //"Tax: " + taxapply;
                    
                    
                    b.Font = new System.Drawing.Font("Cairo", 9, System.Drawing.FontStyle.Regular);
        ...
        protected void b_Click(object sender, EventArgs e)
        {
            ...
            s += "\n Name: ";
            s += b.Name.ToString();
            DecrementStock(b.Tag.ToString(), b);
        ...
        private void DecrementStock(string tag, Button b = null)
        {
            --itemStock[tag];
            string sql = " select  *  from  vw_itemdisplay_sr   where" +

                            " ( product_id like '" + tag + "%'  and product_quantity > 0) ";
            ...
            if (Thread.CurrentThread.CurrentCulture.Parent.Name == "en")
            {
                b.Text += "\n Stock: " + itemStock[b.Tag.ToString()];
                b.Text += "\n R.Price: " + dataReader["retail_price"];
                b.Text += "\n Dis: " + dataReader["discount"] + "% ";
            }
            else
            {
                ...
                //dataReader["product_quantity"]
                b.Text += "\n المخزون: " + itemStock[b.Tag.ToString()];
                b.Text += "\n سعر : " + dataReader["retail_price"];
                b.Text += "\n خصم: " + dataReader["discount"] + "% ";
            }
        }

        private void IncrementStock(string tag, Button b = null)
        {
            ++itemStock[tag];
            string sql = " select  *  from  vw_itemdisplay_sr   where" +
            ...
            else
            {
                try
                {
                   ...
                   // long i = 1;
                    ...
                    string sqlDis = "SELECT disrate FROM storeconfig; ";
                    DataAccess.ExecuteSQL(sqlDis);
                    DataTable dtSql = DataAccess.GetDataTable(sqlDis);
                  ...
                  txtDiscountRate.Text = ShopDisRate;
            ...
            try
            {
                ...
                if (e.ColumnIndex == dgrvSalesItemList.Columns["inc"].Index && e.RowIndex >= 0)
                {
                    foreach (DataGridViewRow row in dgrvSalesItemList.SelectedRows)
                    {
                        ...
                        foreach (Control item in flowLayoutPanelItemList.Controls)
                        {
                            if (item.Tag.ToString() == dgrvSalesItemList.Rows[e.RowIndex].Cells[4].Value.ToString())
                            {
                                DecrementStock(item.Tag.ToString(), (Button)item);
                            }
                        }
                    ...
                    foreach (DataGridViewRow row in dgrvSalesItemList.SelectedRows)
                    {
                        ...
                        if (Convert.ToDouble(row.Cells[2].Value)  >  1)
                        {
                            ...
                            foreach (Control item in flowLayoutPanelItemList.Controls)
                            {
                                if (item.Tag.ToString() == dgrvSalesItemList.Rows[e.RowIndex].Cells[4].Value.ToString())
                                {
                                    IncrementStock(item.Tag.ToString(), (Button)item);
                                }
                            }
                            DataTable dtdisvl = databindsvalues.multiplediscountsvl(product_id);
                            DataRow reader = dtdisvl.Rows[0];
